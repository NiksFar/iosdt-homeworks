...
class ViewController: UIViewController {
    ...
    private func checkSort() {
        if UDService.getKeyStatus(key: Keys.sortKey.rawValue) {
            contents?.sort(by: <)
        } else {
            contents?.sort(by: >)
        }
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let isFile = FileService.shared.checkFile(name: fullName) {
            ...
            if isFile, UDService.getKeyStatus(key: Keys.sizeKey.rawValue) {
                let size = FileService.shared.sizeForLocalFilePath(name: fullName)
                print(size)
                cell.detailTextLabel?.text = "Size: \(size/1024) kbytes"
            }
        }
